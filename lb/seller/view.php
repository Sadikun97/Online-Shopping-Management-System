...
<h2 class =" heading">Seller Information</h2>
...
echo "<table><tr><th>Seller_ID</th><th>Seller_Name</th><th>Designation</th><th>Contact</th><th>Mail_ID</th><th>Action</th><th> Action</th><th>Action</th></tr>";
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				
				
               echo "<tr><td>"  . $row["Seller_ID"]."</td><td>". $row["Seller_Name"]."</td><td>".$row["Designation"]."</td><td>". $row["Contact"]."</td><td>".$row["Mail_ID"]."</td><td><a href='selleredit.php?Seller_ID=".$row['Seller_ID']."' style='text-decoration: none' />Edit</a></td><td><form action='' method='GET'><input type='submit' name='delete".$row['Seller_ID']."' value='Delete'></form></td><td><a href='printseller.php?Seller_ID=".$row['Seller_ID']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";

				// DELETE CODE STARTS FORM HERE
				if(isset($_GET['delete'.$row['Seller_ID']])){
					$delete = "DELETE FROM tb_seller WHERE Seller_ID=".$row['Seller_ID']."";
